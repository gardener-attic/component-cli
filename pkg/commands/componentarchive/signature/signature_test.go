...
var _ = Describe("signature", func() {
	...
	Context("verify", func() {
		...
		It("should fail to verify a digested cd with manipulated resource in child", func() {
			parentCd := getParentCd()
			childCd := getChildCd()
		...
		It("should fail to verify a digested cd with manipulated resource in parent", func() {
			parentCd := getParentCd()
			childCd := getChildCd()
